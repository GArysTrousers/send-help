<script lang="ts">
	...
	const priorityIcons = ['text-blue-500', 'text-yellow-500', 'text-red-500'];
	...
	<Table shadow hoverable={searchedTickets.length > 0} class="w-full">
		...
		<TableBody>
			{#each pagedTickets as t}
				<TableBodyRow
					class="cursor-pointer border-y border-gray-700 bg-cyan-900 {t.assigned.includes(stores.user?.userId || '') ? '' : ''}"
					color={t.assigned.includes(stores.user?.userId || '') ? 'custom' : 'default'}
					onclick={() => onTicketClicked(t.ticketId)}
				>
					<TableBodyCell class="max-w-0">
						<div class="grid min-w-24 max-w-40 grid-cols-3">
							<div>#{t.ticketId}</div>
							<div>
								<Fa icon={faDiamond} class={priorityIcons[t.priority - 1]} />
							</div>
							<div>
								{#if t.risk > 1}
					...
					<TableBodyCell>
						<div class="flex-row gap-0.5 text-lg">
							{#each t.assigned as a}
								<div
									class="h-6 w-6 flex-row items-center justify-center rounded-full border border-gray-500 bg-gray-600"
									title={a}
								>
									{a[0].toUpperCase()}
								</div>
