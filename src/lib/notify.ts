...
export async function notifyTicketCreated(ticketId: number) {
	...
	if (toEmails.length > 0) {
		await sendEmail({
			...
			html: `${owner.fn} ${owner.ln} created a ticket. <a href="${SITE_URL}/tickets?=${ticket.ticketId}">Click here to go to the ticket</a>`,
		});
	}
}
...
export async function notifyTicketUpdated(ticketId: number) {
	...
	await sendEmail({
		...
		html: `Your ticket has been updated. <a href="${SITE_URL}/my?=${ticket.ticketId}">Click here to go to your ticket</a>`,
	});
}
